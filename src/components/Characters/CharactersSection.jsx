...
class CharactersSection extends Component {
  ...
  // Check client window width to make the app responsive-ish, more breakpoints need to be added
  isMobile() {
    return window.innerWidth > 700 ? false : true;
  }
  //Change the characters request offset to the previous 20 characters
  previous() {
  ...
  //Change the characters request offset to the next 20 characters
  next() {
    return this.setState(
      prevState => {
        prevState.config.params.offset =
    ...
    return (
      ...
      >
        {this.state.mounted ? (
          <div>
            <div>
              <RaisedButton label="prev" onClick={e => this.previous(e)} />
              <RaisedButton label="next" onClick={e => this.next(e)} />
            </div>
            <Tabs>
